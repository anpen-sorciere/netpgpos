...
if ($cast_id == '0') {
    ...
    $stmh_cast_ids = $pdo->prepare("
        SELECT cast_id FROM timecard_tbl WHERE shop_mst = ? AND in_ymd = ?
        UNION
    ...
    foreach ($relevant_cast_ids as $current_cast_id) {
        ...
        // 勤務時間・時給の計算
        $stmh_timecard = $pdo->prepare("SELECT * FROM timecard_tbl WHERE cast_id = ? AND shop_mst = ? AND in_ymd = ?");
        $stmh_timecard->execute(array($current_cast_id, $shop_mst, $c_day_format));
        $timecard_data = $stmh_timecard->fetch(PDO::FETCH_ASSOC);

        if ($timecard_data) {
            $in_date_time = $timecard_data['in_ymd'] . ' ' . $timecard_data['in_time'];
            $out_date_time = $timecard_data['out_ymd'] . ' ' . $timecard_data['out_time'];
            $break_start_date_time = $timecard_data['break_start_ymd'] . ' ' . $timecard_data['break_start_time'];
            $break_end_date_time = $timecard_data['break_end_ymd'] . ' ' . $timecard_data['break_end_time'];

            $time1 = strtotime($in_date_time);
            $time2 = strtotime($out_date_time);
            $time3 = strtotime($break_start_date_time);
            $time4 = strtotime($break_end_date_time);

            $net_work_sec = ($time2 - $time1) - ($time4 - $time3);
            $work_time_minutes = floor($net_work_sec / 60);

            $pay_data = pay_get($pdo, $current_cast_id, $ymd[0], $ymd[1]);
            $hourly_wage = $pay_data['pay_amount'];
    ...
    if ($timecard_data) {
        ...
        $work_diff_sec = $time2 - $time1;
        $break_diff_sec = $time4 - $time3;
        $net_work_sec = $work_diff_sec - $break_diff_sec;

        $work_time_minutes = floor($net_work_sec / 60);
        $break_time_minutes = floor($break_diff_sec / 60);

        $pay_data = pay_get($pdo, $cast_id, $ymd[0], $ymd[1]);
        $hourly_wage = $pay_data['pay_amount'];
        $hourly_wage_flg = $pay_data['pay_flg'];
        $jikyu_kingaku = ceil($hourly_wage * ($work_time_minutes / 60));
    }
        ...
        <div class="print_only">
            ...
            <table border="1" cellpadding="5">
                <tbody>
                    ...
                    <tr>
                        <td>
                            <?=htmlspecialchars($timecard_data['in_time'] ?? '')?>
                            ～
                            <?=htmlspecialchars($timecard_data['out_time'] ?? '')?>
                        </td>
                        <td>
                            <?=htmlspecialchars($timecard_data['break_start_time'] ?? '')?>
                            ～
                            <?=htmlspecialchars($timecard_data['break_end_time'] ?? '')?>
                        </td>
