...
try {
    ...
    // クーポンコードがある場合はメッセージ末尾に追記 (プレビュー時も含む)
    // ただし、既にメッセージ中に含まれている場合（手動追記など）は重複を避けるべきだが、
    // ここでは単純に末尾追加とする（運用でカバー）
    // また、custom_message（＝プレビュー編集後）の場合は、
    // ユーザーが意図的に消した可能性もあるので、強制追加は微妙かもしれない。
    // しかし、「特典申請がある場合は必須」という要件から、確実に送ることを優先し、
    // プレビュー生成時（custom_messageが空の時）には追加し、
    // 確定送信時（custom_messageがある時）は、
    // フロント側で「クーポンコード入力欄」と「メッセージ欄」が分かれているため、
    // 最終的に結合して送るのが安全。
    // つまり常に追加するロジックにするか、
    // あるいは「プレビュー生成時のみ追加」して、あとは編集に任せるか。
    // ユーザーフロー：
    // 1. テンプレート選択 -> プレビュー生成 (ここにクーポンが入る)
    // 2. メッセージ編集
    // 3. 送信確定 (編集済みメッセージ + クーポン再追加??)
    // プレビューでメッセージに入っていれば、送信時にはそのメッセージが custom_message として来るので
    // 再追加すると二重になる。
    // なので、「custom_message がない場合（テンプレートから生成時）」のみ追加するのが良さそう。
    // いや、でもテンプレート選択後にクーポン入力したら？
    // JSの `updatePreview` がクーポン入力では発火しない実装になっている（changeイベントのみ）。
    // クーポンは「承認」ボタン押下後に表示され、そこで入力する。
    // その後「送信」を押す。
    // その際、プレビューメッセージ（textarea）は既に生成済み。
    // なので、送信直前にサーバー側で結合するのが確実だが、
    // そうすると「送信内容の確認」でクーポンが見えていない状態になる。
    
    // 解決策:
    // JS側で、クーポンコード入力時にも `updatePreview` を呼ぶように修正する？
    // しかしユーザーは「承認」ボタンを押してモーダルを開いた後にクーポンを入れる。
    // 既にプレビューは表示されている。
    // ユーザーがクーポンを入力してもプレビューは更新されない（現状）。
    
    // 要件：「内容確認画面にクーポン入力欄を表示し、空白ならエラー」
    // 入力させたいということは、それを送りたいということ。
    // シンプルに、送信時にメッセージ末尾に追記する形で実装する。
    // (プレビュー修正などは要件に含まれていないため、確実に送ることを優先)
    
    if ($coupon_code) {
        // 重複チェック（簡易）
        if (strpos($final_message, $coupon_code) === false) {
             $final_message .= "\n\n【クーポンコード】\n" . $coupon_code;
        }
    }
